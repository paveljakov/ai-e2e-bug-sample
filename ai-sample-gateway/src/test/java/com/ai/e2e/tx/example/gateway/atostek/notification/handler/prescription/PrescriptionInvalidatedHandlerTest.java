...
public class PrescriptionInvalidatedHandlerTest {
  ...
  @InjectMocks
  private PrescriptionInvalidatedHandler prescriptionInvalidatedHandler;
  ...
  public void handle() {
    final PrescriptionWithMedication prescriptionWithMedication =
        new PrescriptionWithMedication(prescriptionData, null);
    final Mono<PrescriptionWithMedication> prescriptionWithMedicationMono = Mono.just(prescriptionWithMedication);
    when(prescriptionService.invalidate(eq(prescriptionWithMedication))).thenReturn(prescriptionWithMedicationMono);
    ...
    final NotificationResponseBuilder notificationResponseBuilder =
        prescriptionInvalidatedHandler.handle(notification, new NotificationResponseBuilder(LOG_PREFIX)).block();

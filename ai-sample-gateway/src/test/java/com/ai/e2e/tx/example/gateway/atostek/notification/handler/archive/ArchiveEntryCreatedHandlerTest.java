...
public class ArchiveEntryCreatedHandlerTest {
  ...
  @InjectMocks
  private ArchiveEntryCreatedHandler archiveEntryCreatedHandler;
  ...
  @Test
  public void handle() {
    when(atostekFormService.create(eq(entry))).thenReturn(Mono.just(entry));
    when(notification.getEntry()).thenReturn(entry);

    final NotificationResponse notificationResponse =
        archiveEntryCreatedHandler.handle(notification, new NotificationResponseBuilder(LOG_PREFIX)).block().build();
  ...
  public void handleTemporaryError() {
    when(atostekFormService.create(eq(entry))).thenReturn(Mono.error(ReadTimeoutException.INSTANCE));
    when(notification.getEntry()).thenReturn(entry);
  ...
  public void handlePermanentError() {
    when(atostekFormService.create(eq(entry))).thenReturn(Mono.empty());
    when(notification.getEntry()).thenReturn(entry);
